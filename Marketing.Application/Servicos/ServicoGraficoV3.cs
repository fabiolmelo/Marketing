...
{
    public class ServicoGraficoV3 : IServicoGraficoRevisado
    {
           ...
           using (var image = File.OpenRead(caminhoFundo))
            {
                ...
                using (FileStream filestream = new FileStream(caminhoPdf, FileMode.OpenOrCreate, FileAccess.Write))
                {
                    ...
                    mesReferencia.SetSimpleColumn(mesReferenciaPhrase, 625, 550, 50, 635, 25, Element.ALIGN_LEFT | Element.ALIGN_LEFT);
                    mesReferencia.Go();
